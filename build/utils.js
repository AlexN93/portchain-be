...
function sanitizeStrings(strArray) {
  ...
  return strArray.split(',').map(value => value.trim()).filter((value, index, self) => self.indexOf(value) === index && value !== '');
}

function sanitizeNumbers(numArray) {
  if (numArray === null || typeof numArray !== 'string') return numArray;
  return numArray.split(',').map(value => value.trim() ? Number(value.trim()) : -1).filter(value => !Number.isNaN(value) && value >= 0 && value <= 100).filter((value, index, self) => self.indexOf(value) === index).sort((a, b) => a - b);
}

function hoursBetween(dateFrom, dateTo, absoluteValue = false) {
  if (Object.prototype.toString.call(dateFrom) !== '[object Date]') throw new TypeError('`dateFrom` must be a date object');
  if (Object.prototype.toString.call(dateTo) !== '[object Date]') throw new TypeError('`dateTo` must be a date object');
...
function percentiles(values, percentilePoints) {
  ...
  if (!Array.isArray(percentilePoints) || percentilePoints.length === 0) throw new TypeError('`percentilePoints` must be a non-empty array');
...
function calculatePercentile(values, percentile) {
  ...
  return {
    percentile,
    value: round(fr * (values[ir] - values[ir - 1]) + values[ir - 1], 2)
  };
}
...
function pushSorted(value, comparator) {
  ...
  function insertionIndex(array, value, comparator) {
    ...
    while (low < high) {
      ...
      if (c < 0) low = mid + 1;else if (c > 0) high = mid;else return mid;
    }
